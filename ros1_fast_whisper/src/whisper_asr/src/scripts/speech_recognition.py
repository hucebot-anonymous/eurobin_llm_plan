...
class FasterWhisperASR:
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        if self.log_level == "DEBUG":
            rospy.logdebug(f"[transcribe info] {info}")
...
class OnlineASRProcessor:
    ...
    def __init__(self, asr, log_level, buffer_trimming_sec=15):
    ...
    def init(self):
        ...
        self.transcript_buffer = HypothesisBuffer(self.log_level)
        self.commited = []
    ...
    def process_iter(self):
        ...
        if self.log_level == "DEBUG":
            rospy.logdebug(f"PROMPT: {prompt}")
            rospy.logdebug(f"CONTEXT: {non_prompt}")
            rospy.logdebug(
        ...
        if self.log_level == "DEBUG":
            rospy.logdebug(f">>>>COMPLETE NOW: {completed}")
        the_rest = self.to_flush(self.transcript_buffer.complete())
        if self.log_level == "DEBUG":
            rospy.logdebug(f"INCOMPLETE: {the_rest}")

        if len(self.audio_buffer) / self.SAMPLING_RATE > self.buffer_trimming_sec:
            self.chunk_completed_segment(res)
            if self.log_level == "DEBUG":
                rospy.logdebug("chunking segment")
    ...
    def chunk_completed_segment(self, res):
        ...
        if len(ends) > 1:
            ...
            if e <= t:
                if self.log_level == "DEBUG":
                    rospy.logdebug(f"--- segment chunked at {e:2.2f}")
                self.chunk_at(e)
            else:
                if self.log_level == "DEBUG":
                    rospy.logdebug("--- last segment not within commited area")
        else:
            if self.log_level == "DEBUG":
                rospy.logdebug("--- not enough segments to chunk")
    ...
    def finish(self):
        ...
        f = self.to_flush(o)
        if self.log_level == "DEBUG":
            rospy.logdebug(f"last, noncommited: {f}")
        return f
...
def init_online_whisper(args):
    ...
    if args.model_dir is not None:
        ...
        if args.log_level == "INFO":
            rospy.loginfo(
                f"Loading Whisper {args.model} model for {args.lang}...")
    ...
    e = time.time()
    if args.log_level == "INFO":
        rospy.loginfo(f"done. It took {round(e-t,2)} seconds.")

    # Warm up Whisper
    t = time.time()
    if args.log_level == "INFO":
        rospy.loginfo("Warming up Whisper with random data")
    ...
    return OnlineASRProcessor(asr, args.log_level, buffer_trimming_sec=args.buffer_trimming_sec)


class WhisperPublisher():
    def __init__(self, args):
        rospy.init_node("whisper_publisher", anonymous=True)

        if args.log_level == "INFO":
            rospy.loginfo("Whisper publisher is starting !")

        self.args = args

        self.publisher = rospy.Publisher(args.ros_topic, String, queue_size=10)
    ...
    def pub(self):
        comp_q = queue.Queue()
        online = init_online_whisper(self.args)
        sentence_buffer = ""
        void_count = 0
        ...
        with sd.InputStream(
            ...
            while comp_q.get():
                try:
                    ...
                    rospy.logerr(f"assertion error: {e}")
                else:
                    sentence_buffer += o[2]
                    ...
                    if (
                        ...
                        if self.args.log_level == "INFO":
                            rospy.loginfo(f"ASR msg >> {sentence_buffer}")
                        sentence_buffer = ""
